...
- 设置面板可即时播放 8 个互动动作和“向右走”；向右走默认播放两个步行循环，连续模式会保持到取消勾选
...
│  ├─ Assets/Sprites/01_gaeul_kitsch/  # 125 张动画帧与 2 张拖动形态
...
动画定义集中在 `Services/AnimationCatalog.cs`。角色包含 1 个循环待机动作、8 个非循环互动动作和 1 个移动动作，共 125 张运行时动画帧；idle 的首张静止图停留 2.6 秒，后续眨眼帧按 10 FPS 播放。移动动作按“3 张起步转换帧 + 13 张步态帧循环两次 + 3 张结束转换帧”的顺序以 10 FPS 播放，仅中间 26 张步态时间帧产生位移，前后转换段保持原位；每次随机沿右上或右下 30°移动，并在屏幕工作区四边留出安全间隔。另有 `drag_left.png`、`drag_right.png` 两张静态拖动形态。
...
发布结果生成在 `dist/GaeulDesktopPet-win-x64/`，同时生成对应 ZIP。脚本会验证发布包内恰好包含 127 张运行时 PNG。
